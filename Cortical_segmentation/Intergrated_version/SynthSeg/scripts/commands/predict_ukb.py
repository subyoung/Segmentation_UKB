...
from SynthSeg.predict_ukb import predict
...
parser.add_argument("--resolutionconversion", action='store_true', default=False, dest="resolutionconversion",
                    help="convert the resolution of the input image to 1mm isotropic resolution before segmentation")
parser.add_argument("--keep_intermediate_files", action='store_true', default=False, dest="keep_intermediate_files",
                    help="keep the intermediate files during the resolution conversion")
parser.add_argument("--relabel", action='store_true', default=False, dest="relabel", help="relabel the segmentation")
parser.add_argument("--label_correction", action='store_true', default=False, dest="label_correction", help="using mask to correct the labels in case there is leakage")
parser.add_argument("--save_brain", action='store_true', default=False, dest="save_brain", help="Save the brain mask and brain generated by SynthSeg. If --label_correction is used, the mask and brain generated by SynthSeg may not be necessary, but they can still be saved.") 
parser.add_argument("--output_format", type=str,  default='nii.gz', dest="output_format", help="output format of the segmentation, nii, nii.gz, or img")
parser.add_argument("--save_analyseformat", action='store_true', default = False, dest="save_analyseformat", help="save the segmentation in Analyse format")
